...
var pingCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		bus.SubscribeOnce("user", "system/pong", func(e eventbus.Event) {
			var pong protopkg.PingStatus
			_ = e.Payload.UnmarshalTo(&pong)
			fmt.Println("pong:", pong.Status)
			close(done)
		})

		msg := &protopkg.PingStatus{Status: "ping"}
		payload, _ := anypb.New(msg)
		_ = bus.Publish(eventbus.NewEvent("user", "system/ping", "gapictl", payload, true))
		<-done
	},
}
...
var agentReloadCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		t, _ := transport.NewClientFromConfig(cfg.Transport)
		bus := eventbus.NewEventBus(t)
		evt := eventbus.NewEvent("user", "system/agent.reload", "gapictl", nil, true)
		_ = bus.Publish(evt)
...
var agentStatusCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		bus.SubscribeOnce("user", "system/agents.reply", func(e eventbus.Event) {
		...
		bus.Publish(eventbus.NewEvent("user", "system/agents", "gapictl", packed, true))
		select {
		case <-done:
